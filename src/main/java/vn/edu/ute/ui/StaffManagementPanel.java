...
public class StaffManagementPanel extends JPanel {
    ...
    private JPanel createMainPanel() {
        ...
        txtStaffId = new JTextField(); txtStaffId.setEnabled(false);
        gbc.gridx = 1; gbc.gridy = 0; gbc.weightx = 1.0; formPanel.add(txtStaffId, gbc);

        gbc.gridx = 2; gbc.gridy = 0; gbc.weightx = 0; formPanel.add(new JLabel("Họ tên:"), gbc);
        txtFullName = new JTextField();
        gbc.gridx = 3; gbc.gridy = 0; gbc.weightx = 1.0; formPanel.add(txtFullName, gbc);

        // Row 1
        gbc.gridx = 0; gbc.gridy = 1; gbc.weightx = 0; formPanel.add(new JLabel("SĐT:"), gbc);
        txtPhone = new JTextField();
        gbc.gridx = 1; gbc.gridy = 1; formPanel.add(txtPhone, gbc);

        gbc.gridx = 2; gbc.gridy = 1; formPanel.add(new JLabel("Email:"), gbc);
        txtEmail = new JTextField();
        gbc.gridx = 3; gbc.gridy = 1; formPanel.add(txtEmail, gbc);

        // Row 2
        gbc.gridx = 0; gbc.gridy = 2; formPanel.add(new JLabel("Phân quyền:"), gbc);
        cbRole = new JComboBox<>(new Role[]{Role.Admin, Role.Manager, Role.Accountant});
        gbc.gridx = 1; gbc.gridy = 2; formPanel.add(cbRole, gbc);

        gbc.gridx = 2; gbc.gridy = 2; formPanel.add(new JLabel("Trạng thái:"), gbc);
        cbStatus = new JComboBox<>(Status.values());
        gbc.gridx = 3; gbc.gridy = 2; formPanel.add(cbStatus, gbc);
        ...
        cbFilterRole = new JComboBox<>(new String[]{"Tất cả", "Admin", "Manager", "Accountant"});
        cbFilterStatus = new JComboBox<>(new String[]{"Tất cả", "Active", "Inactive", "Suspended"});

        filterPanel.add(new JLabel("Tìm kiếm (Tên/SĐT):"));
        filterPanel.add(txtSearch);
        filterPanel.add(new JLabel("Phân quyền:"));
        filterPanel.add(cbFilterRole);
        filterPanel.add(new JLabel("Trạng thái:"));
        filterPanel.add(cbFilterStatus);

        txtSearch.getDocument().addDocumentListener(new javax.swing.event.DocumentListener() {
            public void insertUpdate(javax.swing.event.DocumentEvent e) { loadStaffData(); }
            public void removeUpdate(javax.swing.event.DocumentEvent e) { loadStaffData(); }
            public void changedUpdate(javax.swing.event.DocumentEvent e) { loadStaffData(); }
        });
        cbFilterRole.addActionListener(e -> loadStaffData());
        ...
        // --- TABLE ---
        staffTableModel = new DefaultTableModel(new Object[]{"ID", "Họ tên", "SĐT", "Email", "Quyền", "Trạng thái", "Tài khoản"}, 0) {
            @Override
    ...
    private void loadStaffData() {
        try {
            ...
            Role r = null;
            if (cbFilterRole != null && cbFilterRole.getSelectedIndex() > 0) {
                r = Role.valueOf(cbFilterRole.getSelectedItem().toString());
            }
            Status st = null;
            ...
            for (Staff s : list) {
                ...
                staffTableModel.addRow(new Object[]{
                        ...
                        s.getRole().name(), s.getStatus().name(), accountStr
                });
            }
        } catch (Exception ex) {
    ...
    private void fillForm() {
        ...
        if (r >= 0) {
            ...
            cbRole.setSelectedItem(Role.valueOf(tblStaffs.getValueAt(r, 4).toString()));
            cbStatus.setSelectedItem(Status.valueOf(tblStaffs.getValueAt(r, 5).toString()));
    ...
    private void addStaff() {
        try {
            if (txtFullName.getText().isBlank()) throw new Exception("Họ tên không được trống");
            String uname = txtUsername.getText();
            String pwd = new String(txtPassword.getPassword());
            if (uname.isBlank() || pwd.isBlank()) throw new Exception("Vui lòng nhập Username và Password");

            Staff staff = new Staff();
            staff.setFullName(txtFullName.getText().trim());
            staff.setPhone(txtPhone.getText().trim());
            staff.setEmail(txtEmail.getText().trim());
            staff.setRole((Role) cbRole.getSelectedItem());
            staff.setStatus((Status) cbStatus.getSelectedItem());

            staffService.createStaffAccount(staff, uname, pwd);
    ...
    private void updateStaff() {
        try {
            if (txtStaffId.getText().isBlank()) throw new Exception("Vui lòng chọn nhân viên để cập nhật!");
    ...
    private void deleteStaff() {
        try {
            if (txtStaffId.getText().isBlank()) throw new Exception("Vui lòng chọn nhân viên để xóa!");
            int confirm = JOptionPane.showConfirmDialog(this, "Bạn có trê chắn muốn xóa nhân viên này chưa?", "Xác nhận", JOptionPane.YES_NO_OPTION);
